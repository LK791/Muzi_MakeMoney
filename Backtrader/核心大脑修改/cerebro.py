...
class Cerebro(with_metaclass(MetaParams, object)):
    ...
    def load_my_data_from_pickle(self, path="回测缓存文件.pkl"):
        ''' add from pickle'''
        import pickle
    ...
    def runstrategies(self, iterstrat, predata=False):
        ...
        if not predata:
            if self.p.load_my_data:
                ...
                end_time = time.time()
                print("每次预加载都需要消耗时间 :{}".format(end_time - begin_time))
                # assert 0
                ...
                for data in self.datas:
                    ...
                    if self._dopreload:
                        data.preload()
                import pickle
                with open("回测缓存文件.pkl", 'wb') as f:
                    pickle.dump(self.datas, f)
                end_time = time.time()
                print("每次预保存都需要花费时间 :{}".format(end_time - begin_time))
