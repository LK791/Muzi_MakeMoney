...
class Mystrategy(bt.Strategy):
    ...
    def next(self):
        ...
        print(  '今天的开盘价:   ' +  str(self.data0.open[0]))
        print(  '今天的收盘价:   ' +  str(self.data0.close[0]))
        if  self.position.size < 90 :
            print('提交订单')
            self.buy( '000002.SZ', size=10)
            # self.buy( data = '000007.SZ' , size=4000)
        else :
            self.sell( '000002.SZ', size=90)
        ...
        for i in  posdata :
            ...
            print( 'trade里的pnl' , self._trades[self.getdatabyname(i._name)][0][-1].pnl             )
        print()
